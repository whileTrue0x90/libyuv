...
#define TESTQPLANARTOPI(FUNC, STYPE, DTYPE, DEPTH, W1280, N, NEG, OFF)      \
  TEST_F(LibYUVPlanarTest, FUNC##Plane_##DEPTH##N) {                        \
    ...
    for (int i = 0; i < kPixels; ++i) {                                     \
      src_pixels_r[i] = fastrand() & ((1 << DEPTH) - 1);                    \
      src_pixels_g[i] = fastrand() & ((1 << DEPTH) - 1);                    \
      src_pixels_b[i] = fastrand() & ((1 << DEPTH) - 1);                    \
      src_pixels_a[i] = fastrand() & ((1 << DEPTH) - 1);                    \
    }                                                                       \
...
TESTTPLANARTOP(MergeAR30, uint16_t, uint8_t, 10)
TESTTPLANARTOP(MergeAR30, uint16_t, uint8_t, 12)
TESTTPLANARTOP(MergeAR30, uint16_t, uint8_t, 16)
