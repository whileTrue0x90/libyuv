...
//    (reinterpret_cast<float*>(orig_y))[i]  = i * 3.1415f;
//  }
  ...
  for (j = 0; j < benchmark_iterations; j++) {
    ...
    max_opt = ScaleSumSamples_NEON(reinterpret_cast<float*>(orig_y),
                                   reinterpret_cast<float*>(dst_opt),
                                   scale,
                                   benchmark_width * benchmark_height);
#else
...
TEST_F(LibYUVPlanarTest, TestScaleSumSamples) {
  int diff = TestScaleSumSamples(benchmark_width_, benchmark_height_,
                                 benchmark_iterations_, 1.2f);
  EXPECT_EQ(0, diff);
}
