...
TEST_F(LibYUVPlanarTest, Convert8To16Row_Opt) {
  ...
  Convert8To16Row_C(src_pixels_y, reinterpret_cast<uint16*>(dst_pixels_y_c), 1024, kPixels);

  int has_avx2 = TestCpuFlag(kCpuHasAVX2);
  int has_ssse3 = TestCpuFlag(kCpuHasSSSE3);
  for (int i = 0; i < benchmark_iterations_; ++i) {
    if (has_avx2) {
      Convert8To16Row_AVX2(src_pixels_y, reinterpret_cast<uint16*>(dst_pixels_y_opt), 1024, kPixels);
    } else if (has_ssse3) {
      Convert8To16Row_SSSE3(src_pixels_y, reinterpret_cast<uint16*>(dst_pixels_y_opt), 1024, kPixels);
    } else {
      Convert8To16Row_C(src_pixels_y, reinterpret_cast<uint16*>(dst_pixels_y_opt), 1024, kPixels);
    }
  }
