...
#define TESTATOBI(FMT_A, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, FMT_B, TYPE_B,   \
                  BPP_B, STRIDE_B, HEIGHT_B, W1280, N, NEG, OFF)             \
  TEST_F(LibYUVConvertTest, FMT_A##To##FMT_B##N) {                           \
    ...
    MaskCpuFlags(disable_cpu_flags_);                                        \
    FMT_A##To##FMT_B((TYPE_A*)(src_argb + OFF), kStrideA / sizeof(TYPE_A),   \
                     (TYPE_B*)dst_argb_c, kStrideB / sizeof(TYPE_B), kWidth, \
                     NEG kHeight);                                           \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    for (int i = 0; i < benchmark_iterations_; ++i) {                        \
      FMT_A##To##FMT_B((TYPE_A*)(src_argb + OFF), kStrideA / sizeof(TYPE_A), \
                       (TYPE_B*)dst_argb_opt, kStrideB / sizeof(TYPE_B),     \
                       kWidth, NEG kHeight);                                 \
    }                                                                        \
...
#define TESTATOBRANDOM(FMT_A, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, FMT_B,        \
                       TYPE_B, BPP_B, STRIDE_B, HEIGHT_B)                      \
  TEST_F(LibYUVConvertTest, FMT_A##To##FMT_B##_Random) {                       \
    for (int times = 0; times < benchmark_iterations_; ++times) {              \
      ...
      MaskCpuFlags(disable_cpu_flags_);                                        \
      FMT_A##To##FMT_B((TYPE_A*)src_argb, kStrideA / sizeof(TYPE_A),           \
                       (TYPE_B*)dst_argb_c, kStrideB / sizeof(TYPE_B), kWidth, \
                       kHeight);                                               \
      MaskCpuFlags(benchmark_cpu_info_);                                       \
      FMT_A##To##FMT_B((TYPE_A*)src_argb, kStrideA / sizeof(TYPE_A),           \
...
#define TESTATOB(FMT_A, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, FMT_B, TYPE_B,   \
                 BPP_B, STRIDE_B, HEIGHT_B)                                 \
  TESTATOBI(FMT_A, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, FMT_B, TYPE_B, BPP_B, \
  ...
  TESTATOBRANDOM(FMT_A, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, FMT_B, TYPE_B,   \
                 BPP_B, STRIDE_B, HEIGHT_B)
...
TESTATOB(ARGB, uint8_t, 4, 4, 1, AR64, uint16_t, 8, 8, 1)
TESTATOB(ARGB, uint8_t, 4, 4, 1, AB64, uint16_t, 8, 8, 1)
TESTATOB(ABGR, uint8_t, 4, 4, 1, AR64, uint16_t, 8, 8, 1)
TESTATOB(ABGR, uint8_t, 4, 4, 1, AB64, uint16_t, 8, 8, 1)
TESTATOB(AR64, uint16_t, 8, 8, 1, ARGB, uint8_t, 4, 4, 1)
TESTATOB(AB64, uint16_t, 8, 8, 1, ARGB, uint8_t, 4, 4, 1)
TESTATOB(AR64, uint16_t, 8, 8, 1, ABGR, uint8_t, 4, 4, 1)
TESTATOB(AB64, uint16_t, 8, 8, 1, ABGR, uint8_t, 4, 4, 1)
TESTATOB(AR64, uint16_t, 8, 8, 1, AB64, uint16_t, 8, 8, 1)
TESTATOB(AB64, uint16_t, 8, 8, 1, AR64, uint16_t, 8, 8, 1)
...
#define TESTSYMI(FMT_ATOB, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, W1280, N, NEG, \
                 OFF)                                                        \
  TEST_F(LibYUVConvertTest, FMT_ATOB##_Symetric##N) {                        \
    ...
    MaskCpuFlags(disable_cpu_flags_);                                        \
    FMT_ATOB((TYPE_A*)(src_argb + OFF), kStrideA / sizeof(TYPE_A),           \
             (TYPE_A*)dst_argb_c, kStrideA / sizeof(TYPE_A), kWidth,         \
             NEG kHeight);                                                   \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    for (int i = 0; i < benchmark_iterations_; ++i) {                        \
      FMT_ATOB((TYPE_A*)(src_argb + OFF), kStrideA / sizeof(TYPE_A),         \
               (TYPE_A*)dst_argb_opt, kStrideA / sizeof(TYPE_A), kWidth,     \
               NEG kHeight);                                                 \
    }                                                                        \
    MaskCpuFlags(disable_cpu_flags_);                                        \
    FMT_ATOB((TYPE_A*)dst_argb_c, kStrideA / sizeof(TYPE_A),                 \
             (TYPE_A*)dst_argb_c, kStrideA / sizeof(TYPE_A), kWidth,         \
             NEG kHeight);                                                   \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    FMT_ATOB((TYPE_A*)dst_argb_opt, kStrideA / sizeof(TYPE_A),               \
             (TYPE_A*)dst_argb_opt, kStrideA / sizeof(TYPE_A), kWidth,       \
             NEG kHeight);                                                   \
    for (int i = 0; i < kStrideA * kHeightA; ++i) {                          \
...
#define TESTSYM(FMT_ATOB, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A)                  \
  TESTSYMI(FMT_ATOB, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, benchmark_width_ - 4, \
           _Any, +, 0)                                                        \
  TESTSYMI(FMT_ATOB, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, benchmark_width_,     \
           _Unaligned, +, 1)                                                  \
  TESTSYMI(FMT_ATOB, TYPE_A, BPP_A, STRIDE_A, HEIGHT_A, benchmark_width_,     \
           _Opt, +, 0)
...
TESTSYM(AB64ToAR64, uint16_t, 8, 8, 1)
TESTSYM(AR64ToAB64, uint16_t, 8, 8, 1)
