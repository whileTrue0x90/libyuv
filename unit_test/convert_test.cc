...
TEST_F(LibYUVConvertTest, TestH010ToAR30) {
  ...
  AR30ToARGB(ar30_pixels, 0, argb_pixels, 0, kSize, 1);

  for (int i = 0; i < kSize; ++i) {
  	int b10 = ((uint32*)ar30_pixels)[i] & 1023;
  	int g10 = (((uint32*)ar30_pixels)[i] >> 10) & 1023;
  	int r10 = (((uint32*)ar30_pixels)[i] >> 20) & 1023;
  	++histogram_b[b10];
  	++histogram_g[g10];
  	++histogram_r[r10];
    int expected_y = Clamp(static_cast<int>((i - 64) * 1.164f / 4));
  ...
  int count_r = 0;
  for (int i = 0; i < kSize; ++i) {
  	if (histogram_b[i]) ++count_b;
  	if (histogram_g[i]) ++count_g;
  	if (histogram_r[i]) ++count_r;
  }
