...
#define TESTATOBI(FMT_A, TYPE_A, EPP_A, STRIDE_A, HEIGHT_A, FMT_B, TYPE_B,   \
  ...
  TEST_F(LibYUVConvertTest, FMT_A##To##FMT_B##N) {                           \
    ...
    align_buffer_page_end(src_argb, kStrideA* kHeightA* sizeof(TYPE_A) + OFF);               \
    ...
    FMT_A##To##FMT_B((TYPE_A*)(src_argb + OFF), kStrideA,   \
                     (TYPE_B*)dst_argb_c, kStrideB, kWidth, \
                     NEG kHeight);                                           \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    for (int i = 0; i < benchmark_iterations_; ++i) {                        \
      FMT_A##To##FMT_B((TYPE_A*)(src_argb + OFF), kStrideA, \
                       (TYPE_B*)dst_argb_opt, kStrideB,     \
                       kWidth, NEG kHeight);                                 \
    }                                                                        \
...
#define TESTATOBRANDOM(FMT_A, TYPE_A, EPP_A, STRIDE_A, HEIGHT_A, FMT_B,        \
  ...
  TEST_F(LibYUVConvertTest, FMT_A##To##FMT_B##_Random) {                       \
    for (int times = 0; times < benchmark_iterations_; ++times) {              \
      ...
      MaskCpuFlags(disable_cpu_flags_);                                        \
      FMT_A##To##FMT_B((TYPE_A*)src_argb, kStrideA,           \
                       (TYPE_B*)dst_argb_c, kStrideB, kWidth, \
                       kHeight);                                               \
      MaskCpuFlags(benchmark_cpu_info_);                                       \
      FMT_A##To##FMT_B((TYPE_A*)src_argb, kStrideA,           \
...
#define TESTSYMI(FMT_ATOB, TYPE_A, EPP_A, STRIDE_A, HEIGHT_A, W1280, N, NEG, \
  ...
  TEST_F(LibYUVConvertTest, FMT_ATOB##_Symetric##N) {                        \
    ...
    MaskCpuFlags(disable_cpu_flags_);                                        \
    FMT_ATOB((TYPE_A*)(src_argb + OFF), kStrideA,           \
             (TYPE_A*)dst_argb_c, kStrideA, kWidth,         \
             NEG kHeight);                                                   \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    for (int i = 0; i < benchmark_iterations_; ++i) {                        \
      FMT_ATOB((TYPE_A*)(src_argb + OFF), kStrideA,         \
               (TYPE_A*)dst_argb_opt, kStrideA, kWidth,     \
               NEG kHeight);                                                 \
    }                                                                        \
    MaskCpuFlags(disable_cpu_flags_);                                        \
    FMT_ATOB((TYPE_A*)dst_argb_c, kStrideA,                 \
             (TYPE_A*)dst_argb_c, kStrideA, kWidth,         \
             NEG kHeight);                                                   \
    MaskCpuFlags(benchmark_cpu_info_);                                       \
    FMT_ATOB((TYPE_A*)dst_argb_opt, kStrideA,               \
             (TYPE_A*)dst_argb_opt, kStrideA, kWidth,       \
             NEG kHeight);                                                   \
    for (int i = 0; i < kStrideA * kHeightA; ++i) {                          \
...
TESTSYM(AR64ToAB64, uint16_t, 4, 4, 1)
