...
#define TESTPLANARTOPI(SRC_FMT_PLANAR, SRC_T, SRC_BPC, SRC_SUBSAMP_X,        \
  ...
  TEST_F(LibYUVConvertTest, SRC_FMT_PLANAR##To##FMT_PLANAR##N) {              \
    ...
    for (int i = 0; i < kHeight; ++i)                                         \
      for (int j = 0; j < kWidth * SRC_BPC; ++j)                              \
        src_y[i * kWidth * SRC_BPC + j + OFF] = (fastrand() & 0xff);          \
    for (int i = 0; i < SUBSAMPLE(kHeight, SRC_SUBSAMP_Y); ++i) {             \
      for (int j = 0; j < SUBSAMPLE(kWidth, SRC_SUBSAMP_X) * SRC_BPC; ++j) {  \
        src_u[(i * SUBSAMPLE(kWidth, SRC_SUBSAMP_X)) * SRC_BPC + j + OFF] =   \
            (fastrand() & 0xff);                                              \
        src_v[(i * SUBSAMPLE(kWidth, SRC_SUBSAMP_X)) * SRC_BPC + j + OFF] =   \
            (fastrand() & 0xff);                                              \
      }                                                                       \
    }                                                                         \
...
#define TESTPLANARTOP(SRC_FMT_PLANAR, SRC_T, SRC_BPC, SRC_SUBSAMP_X,        \
                       SRC_SUBSAMP_Y, FMT_PLANAR, DST_T, DST_BPC, SUBSAMP_X, \
                       SUBSAMP_Y)                                            \
  TESTPLANARTOPI(SRC_FMT_PLANAR, SRC_T, SRC_BPC, SRC_SUBSAMP_X,             \
                  SRC_SUBSAMP_Y, FMT_PLANAR, DST_T, DST_BPC, SUBSAMP_X,      \
