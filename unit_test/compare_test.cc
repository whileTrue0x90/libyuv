...
TEST_F(LibYUVCompareTest, TestHammingDistance_Opt) {
  ...
  uint32 h0 = HammingDistance_C(src_a, src_b, benchmark_width_ * benchmark_height_);
  EXPECT_EQ(benchmark_width_ * benchmark_height_ * 8ULL, h0);

  uint32 h2 = ComputeHammingDistance(src_a, src_b, benchmark_width_ * benchmark_height_);
  EXPECT_EQ(benchmark_width_ * benchmark_height_ * 8ULL, h2);


  for (int i = 0; i < benchmark_iterations_; ++i) {
#if defined(HAS_HAMMINGDISTANCE_NEON)
    h1 = HammingDistance_NEON(src_a, src_b, benchmark_width_ * benchmark_height_);
#elif defined(HAS_HAMMINGDISTANCE_AVX2)
    int has_avx2 = TestCpuFlag(kCpuHasAVX2);
    if (has_avx2) {
      h1 = HammingDistance_AVX2(src_a, src_b, benchmark_width_ * benchmark_height_);
    } else {
      int has_ssse3 = TestCpuFlag(kCpuHasSSSE3);
      if (has_ssse3) {
        h1 = HammingDistance_SSSE3(src_a, src_b, benchmark_width_ * benchmark_height_);
      } else {
        h1 = HammingDistance_X86(src_a, src_b, benchmark_width_ * benchmark_height_);
      }
    }
#elif defined(HAS_HAMMINGDISTANCE_X86)
    h1 = HammingDistance_X86(src_a, src_b, benchmark_width_ * benchmark_height_);
  ...
  if ((benchmark_width_ * benchmark_height_) <= kMaxOptCount) {
    ...
    printf("warning - HammingDistance_Opt does not match HammingDistance_C: "
