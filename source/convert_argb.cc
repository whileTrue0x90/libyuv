...
static int H010ToARGBMatrix(const uint16* src_y,
  ...
  void (*I422ToARGBRow)(const uint8* y_buf, const uint8* u_buf,
                        ...
                        const struct YuvConstants* yuvconstants, int width) =
      I422ToARGBRow_C;
  void (*ARGBToARGBRow)(const uint8* src_argb, uint8* dst_rgb, int width) =
      ARGBToARGBRow_C;
...
#if defined(HAS_ARGBTOARGBROW_AVX2)
  if (TestCpuFlag(kCpuHasAVX2)) {
    ARGBToARGBRow = ARGBToARGBRow_Any_AVX2;
    if (IS_ALIGNED(width, 8)) {
      ARGBToARGBRow = ARGBToARGBRow_AVX2;
    }
  }
#endif
