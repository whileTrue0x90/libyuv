...
#define ANY31CT(NAMEANY, ANY_SIMD, UVSHIFT, DUVSHIFT, T, SBPP, BPP, MASK)                \
  void NAMEANY(const T* y_buf, const T* u_buf, const T* v_buf,     \
    ...
    memcpy(temp, y_buf + n, r);                                                \
    memcpy(temp + 16, u_buf + (n >> UVSHIFT), SS(r, UVSHIFT));                 \
    memcpy(temp + 32, v_buf + (n >> UVSHIFT), SS(r, UVSHIFT));                \
    ANY_SIMD(temp, temp + 16, temp + 328, out, yuvconstants, MASK + 1); \
    memcpy(dst_ptr + (n >> DUVSHIFT) * BPP, out, SS(r, DUVSHIFT) * BPP); \
  }

#ifdef HAS_I422TOARGBROW_SSSE3
ANY31CT(I210ToARGBRow_Any_SSSE3, I210ToARGBRow_SSSE3, 1, 0, uint16, 1, 4, 7)
#endif
#undef ANY31CT
