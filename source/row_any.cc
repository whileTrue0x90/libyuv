...
#define ANYDETILEMERGE(NAMEANY, ANY_SIMD, MASK)                                \
  void NAMEANY(const uint8_t* src_y, ptrdiff_t src_y_tile_stride,              \
    ...
    int n = width & ~MASK;                                                     \
    if (n > 0) {                                                               \
      ANY_SIMD(src_y, src_y_tile_stride, src_uv, src_uv_tile_stride, dst_yuyv, \
               n);                                                             \
    }                                                                          \
    ...
    ANY_SIMD(temp, src_y_tile_stride, temp + 16, src_uv_tile_stride,           \
             temp + 32, r);                                                    \
    memcpy(dst_yuyv + 2 * n, temp + 32, 2 * r);                                \
  }

#ifdef HAS_DETILEMERGEROW_NEON
ANYDETILEMERGE(DetileMergeRow_Any_NEON, DetileMergeRow_NEON, 15)
#endif
