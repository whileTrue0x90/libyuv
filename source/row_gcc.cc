...
// I410 shuf 8U+8V to 8UV
static const lvec8 kShuffleUVInterleave = {
...
// shift_left: 16 - bitdepth, shift_right: bitdepth - 8
#define READYUV21X(shift_left, shift_right)                       \
  "movq       (%[u_buf]),%%xmm0                               \n" \
  "movq       0x00(%[u_buf],%[v_buf],1),%%xmm1                \n" \
  "lea        0x8(%[u_buf]),%[u_buf]                          \n" \
  "punpcklwd  %%xmm1,%%xmm0                                   \n" \
  "psraw      $" #shift_right ",%%xmm0                        \n" \
  "packuswb   %%xmm0,%%xmm0                                   \n" \
  "punpcklwd  %%xmm0,%%xmm0                                   \n" \
  "movdqu     (%[y_buf]),%%xmm4                               \n" \
  "psllw      $" #shift_left ",%%xmm4                         \n" \
  "lea        0x10(%[y_buf]),%[y_buf]                         \n"

#define READYUVA21X(shift_left, shift_right)                      \
  "movq       (%[u_buf]),%%xmm0                               \n" \
  "movq       0x00(%[u_buf],%[v_buf],1),%%xmm1                \n" \
  "lea        0x8(%[u_buf]),%[u_buf]                          \n" \
  "punpcklwd  %%xmm1,%%xmm0                                   \n" \
  ...
  "lea        0x10(%[y_buf]),%[y_buf]                         \n" \
  "movdqu     (%[a_buf]),%%xmm5                               \n" \
  "psraw      $" #shift_right ",%%xmm5                        \n" \
  "packuswb   %%xmm5,%%xmm5                                   \n" \
  "lea        0x10(%[a_buf]),%[a_buf]                         \n"

// Read 8 UV from 444 10 bit
#define READYUV41X(shift_left, shift_right)                       \
  "movdqu     (%[u_buf]),%%xmm0                               \n" \
  "movdqu     0x00(%[u_buf],%[v_buf],1),%%xmm1                \n" \
  "lea        0x10(%[u_buf]),%[u_buf]                         \n" \
  "psraw      $" #shift_right ",%%xmm0                        \n" \
  "psraw      $" #shift_right ",%%xmm1                        \n" \
  "packuswb   %%xmm1,%%xmm0                                   \n" \
  "pshufb     %[kShuffleUVInterleave], %%xmm0                 \n" \
  "movdqu     (%[y_buf]),%%xmm4                               \n" \
  "psllw      $" #shift_left ",%%xmm4                         \n" \
  "lea        0x10(%[y_buf]),%[y_buf]                         \n"

// Read 8 UV from 444 10 bit.  With 8 Alpha.
#define READYUVA41X(shift_left, shift_right)                      \
  "movdqu     (%[u_buf]),%%xmm0                               \n" \
  "movdqu     0x00(%[u_buf],%[v_buf],1),%%xmm1                \n" \
  "lea        0x10(%[u_buf]),%[u_buf]                         \n" \
  "psraw      $" #shift_right ",%%xmm0                        \n" \
...
void OMITFP I410ToARGBRow_SSSE3(const uint16_t* y_buf,
  ...
  : [y_buf]"+r"(y_buf),    // %[y_buf]
    ...
    [width]"+rm"(width)    // %[width]
  : [yuvconstants]"r"(yuvconstants),  // %[yuvconstants]
    [kShuffleUVInterleave]"m"(kShuffleUVInterleave)
...
#define READYUV21X_AVX2(shift_left, shift_right)                   \
  ...
  "vpsraw     $" #shift_right ",%%ymm0,%%ymm0                  \n" \
  "vpackuswb  %%ymm0,%%ymm0,%%ymm0                             \n" \
  "vpunpcklwd %%ymm0,%%ymm0,%%ymm0                             \n" \
  "vmovdqu    (%[y_buf]),%%ymm4                                \n" \
  "vpsllw     $" #shift_left ",%%ymm4,%%ymm4                   \n" \
  "lea        0x20(%[y_buf]),%[y_buf]                          \n"

// Read 8 UV from 210 9bit - 15bit, upsample to 16 UV. With 16 Alpha.
#define READYUVA21X_AVX2(shift_left, shift_right)                  \
  "vmovdqu    (%[u_buf]),%%xmm0                                \n" \
  "vmovdqu    0x00(%[u_buf],%[v_buf],1),%%xmm1                 \n" \
  "lea        0x10(%[u_buf]),%[u_buf]                          \n" \
  "vpermq     $0xd8,%%ymm0,%%ymm0                              \n" \
  ...
  "lea        0x20(%[y_buf]),%[y_buf]                          \n" \
  "vmovdqu    (%[a_buf]),%%ymm5                                \n" \
  "vpsraw     $" #shift_right ",%%ymm5,%%ymm5                  \n" \
  "vpackuswb  %%ymm5,%%ymm5,%%ymm5                             \n" \
  "lea        0x20(%[a_buf]),%[a_buf]                          \n"

// Read 16 UV from 410 9bit - 15bit
#define READYUV41X_AVX2(shift_left, shift_right)                   \
  "vmovdqu    (%[u_buf]),%%ymm0                                \n" \
  "vmovdqu    0x00(%[u_buf],%[v_buf],1),%%ymm1                 \n" \
  "lea        0x20(%[u_buf]),%[u_buf]                          \n" \
  "vpsraw     $" #shift_right ",%%ymm0,%%ymm0                  \n" \
  "vpsraw     $" #shift_right ",%%ymm1,%%ymm1                  \n" \
  "vpackuswb  %%ymm1,%%ymm0,%%ymm0                             \n" \
  "vpshufb    %[kShuffleUVInterleave],%%ymm0,%%ymm0            \n" \
  "vmovdqu    (%[y_buf]),%%ymm4                                \n" \
  "vpsllw     $" #shift_left ",%%ymm4,%%ymm4                   \n" \
  "lea        0x20(%[y_buf]),%[y_buf]                          \n"

// Read 16 UV from 410 9bit - 15bit. With 16 Alpha.
#define READYUVA41X_AVX2(shift_left, shift_right)                  \
  "vmovdqu    (%[u_buf]),%%ymm0                                \n" \
  "vmovdqu    0x00(%[u_buf],%[v_buf],1),%%ymm1                 \n" \
  "lea        0x20(%[u_buf]),%[u_buf]                          \n" \
  "vpsraw     $" #shift_right ",%%ymm0,%%ymm0                  \n" \
