...
__attribute__((target("avx2"))) void ARGBToRGB24Row_AVX2(const uint8_t* src,
                                                         uint8_t* dst,
                                                         int width) {
  for (int i = 0; i < width; i += 8) {
    ...
    __m256i rgb_weird = _mm256_shuffle_epi8(
        rgba,
        _mm256_setr_epi8(0, 1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, _, _, _, _, 0,
                         1, 2, 4, 5, 6, 8, 9, 10, 12, 13, 14, _, _, _, _));

    __m256i rgb = _mm256_permutevar8x32_epi32(
        rgb_weird, _mm256_setr_epi32(0, 1, 2, 4, 5, 6, 3, 7));
