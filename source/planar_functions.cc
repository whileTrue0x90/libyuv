...
LIBYUV_API
void MergeAR30Plane(const uint16_t* src_r,
                    int src_stride_r,
                    ...
                    int depth) {
  int y;
  void (*MergeAR30Row)(const uint16_t* src_r, const uint16_t* src_g,
                       const uint16_t* src_b, uint8_t* dst_ar30, int depth,
                       int width) = MergeAR30Row_C;
...
#if defined(HAS_MERGEAR30ROW_AVX2)
  if (TestCpuFlag(kCpuHasAVX2)) {
    MergeAR30Row = MergeAR30Row_Any_AVX2;
    if (IS_ALIGNED(width, 16)) {
      MergeAR30Row = MergeAR30Row_AVX2;
    }
  }
#endif
#if defined(HAS_MERGEAR30ROW_NEON)
  if (TestCpuFlag(kCpuHasNEON)) {
    MergeAR30Row = MergeAR30Row_Any_NEON;
    if (IS_ALIGNED(width, 8)) {
      MergeAR30Row = MergeAR30Row_NEON;
    }
  }
#endif

  for (y = 0; y < height; ++y) {
    MergeAR30Row(src_r, src_g, src_b, dst_ar30, depth, width);
    src_r += src_stride_r;
    src_g += src_stride_g;
    src_b += src_stride_b;
