...
void ScaleARGBCols_MMI(uint8_t* dst_argb,
  ...
  __asm__ volatile(
      "1:                                                           \n\t"
      "dsra       %[offset],        %[x],             %[shift0]     \n\t"
      "dsll       %[offset],        %[offset],        %[shift1]     \n\t"
      ...
      "bnez       %[width],         1b                              \n\t"
      : [dest] "=&f"(dest), [offset] "=&r"(offset)
      : [src_ptr] "r"(src), [dst_ptr] "r"(dst), [src_tmp] "r"(src_tmp),
        [width] "r"(dst_width), [dx] "r"(dx), [x] "r"(x), [shift0] "r"(shift0),
        [shift1] "r"(shift1)
      : "memory");
}
