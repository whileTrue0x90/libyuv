...
uint32 HammingDistance_MSA(const uint8* src_a, const uint8* src_b, int count) {
  ...
  for (i = 0; i < count; i += 32) {
    ...
    vec0 += vec1;
    reg0 = __msa_pckev_w((v4i32)vec0, (v4i32)vec0);
    vec0 = __msa_hadd_s_d(reg0, reg0);
    diff += (uint32)vec0[0];
    src_a += 32;
    src_b += 32;
  }
...
uint32 SumSquareError_MSA(const uint8* src_a, const uint8* src_b, int count) {
  ...
  for (i = 0; i < count; i += 32) {
    ...
    reg0 = __msa_dotp_s_w(vec0, vec0);
    reg1 = __msa_dotp_s_w(vec1, vec1);
    reg2 = __msa_dotp_s_w(vec2, vec2);
    reg3 = __msa_dotp_s_w(vec3, vec3);
    reg0 += reg1;
    reg2 += reg3;
    reg0 += reg2;
    tmp0 = __msa_hadd_s_d(reg0, reg0);
    reg0 = __msa_pckev_w((v4i32)tmp0, (v4i32)tmp0);
    tmp0 = __msa_hadd_s_d(reg0, reg0);
    sse += tmp0[0];
    src_a += 32;
    src_b += 32;
