...
LIBYUV_API
int MT2TToP010(const uint8_t* src_y, int src_stride_y, const uint8_t* src_uv,
               int src_stride_uv, uint16_t* dst_y, int dst_stride_y,
               uint16_t* dst_uv, int dst_stride_uv, int width, int height) {
...

  {
    ...
    const int uv_tile_height = y_tile_height / 2;
    ...
    if (height < 0) {
      ...
      detile_dir = -1;
      dst_uv = dst_uv + (uv_height - uv_tile_height) * dst_stride_uv;
      if (src_y && dst_y) {
        dst_y = dst_y + (height - y_tile_height) * dst_stride_y;
      }
    }

    // Unpack and detile Y plane
    if (src_y && dst_y) {
      for (y = 0; y < (height & ~(y_tile_height - 1)); y += y_tile_height) {
        ...
        DetilePlane_16((uint16_t*)row_buf, padded_width, dst_y, dst_stride_y,
                       width, y_tile_height * detile_dir, y_tile_height);
        src_y += src_stride_y * y_tile_height;
        dst_y += dst_stride_y * y_tile_height * detile_dir;
      }
      if (height & (y_tile_height - 1)) {
        if (detile_dir < 0)
          dst_y +=
              dst_stride_y * (y_tile_height - (height & (y_tile_height - 1)));
        UnpackMT2T(src_y, (uint16_t*)row_buf, y_tile_row_size);
        DetilePlane_16((uint16_t*)row_buf, padded_width, dst_y, dst_stride_y,
                       width, (height & (y_tile_height - 1)) * detile_dir,
                       y_tile_height);
      }
    }
    ...
    for (y = 0; y < (uv_height & ~(uv_tile_height - 1)); y += uv_tile_height) {
      ...
      DetilePlane_16((uint16_t*)row_buf, padded_width, dst_uv, dst_stride_uv,
                     uv_width, uv_tile_height * detile_dir,
                     uv_tile_height);
      src_uv += src_stride_uv * uv_tile_height;
      dst_uv += dst_stride_uv * uv_tile_height * detile_dir;
    }
    if (uv_height & (uv_tile_height - 1)) {
      if (detile_dir < 0)
        dst_uv += dst_stride_uv *
                 (uv_tile_height - (uv_height & (uv_tile_height - 1)));
      UnpackMT2T(src_uv, (uint16_t*)row_buf, uv_tile_row_size);
      DetilePlane_16((uint16_t*)row_buf, padded_width, dst_uv, dst_stride_uv,
                     uv_width,
                     (uv_height & (uv_tile_height - 1)) * detile_dir,
                     uv_tile_height);
    }
