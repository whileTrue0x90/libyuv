...
static void ScaleUVDownEven(int src_width,
  ...
  if (TestCpuFlag(kCpuHasMMI)) {
    ScaleUVRowDownEven = filtering ? ScaleUVRowDownEvenBox_Any_MMI
  ...
  if (TestCpuFlag(kCpuHasMSA)) {
    ScaleUVRowDownEven = filtering ? ScaleUVRowDownEvenBox_Any_MSA
...
static void ScaleUVBilinearUp(int src_width,
  ...
  if (src_width >= 32768) {
    ScaleUVFilterCols =
        filtering ? ScaleUVFilterCols64_C : ScaleUVCols64_C;
  }
#if defined(HAS_SCALEUVFILTERCOLS_SSSE3)
...
static void ScaleUVSimple(int src_width,
                            ...
                            int dy) {
  int j;
  void (*ScaleUVCols)(uint8_t * dst_uv, const uint8_t* src_uv,
                        int dst_width, int x, int dx) =
...
static int UVCopy(const uint8_t* src_UV,
  ...
  CopyPlane(src_UV, src_stride_UV, dst_UV, dst_stride_UV, width * 2,
            height);
  return 0;
}
#endif  // HAS_UVCOPY
  ...
  if (((dx | dy) & 0xffff) == 0) {
    if (!dx || !dy) {  // 1 pixel wide and/or tall.
      ...
      if ((dx & 0x10000) && (dy & 0x10000)) {
        ...
        if (dx == 0x10000 && dy == 0x10000) {
          // Straight copy.
          UVCopy(src + (y >> 16) * src_stride + (x >> 16) * 2, src_stride,
...
int UVScale(const uint8_t* src_uv,
  ...
  ScaleUV(src_uv, src_stride_uv, src_width, src_height, dst_uv,
            dst_stride_uv, dst_width, dst_height, 0, 0, dst_width, dst_height,
            filtering);
  return 0;
}
