...
void SobelXRow_MMI(const uint8_t* src_y0,
  ...
  __asm__ volatile(
      ...
      "bgtz      %[width],      1b                       \n\t"
      "nop                                               \n\t"
      : [sobel] "+f"(sobel), [src_y0] "+r"(src_y0), [src_y1] "+r"(src_y1),
        [src_y2] "+r"(src_y2), [width] "+r"(width),
        [dst_sobelx] "+r"(dst_sobelx), [y00] "+f"(y00), [y10] "+f"(y10),
...
void SobelYRow_MMI(const uint8_t* src_y0,
  ...
  __asm__ volatile(
      ...
      "nop                                              \n\t"
      : [sobel] "+f"(sobel), [src_y0] "+r"(src_y0), [src_y1] "+r"(src_y1),
        [width] "+r"(width), [dst_sobely] "+r"(dst_sobely), [y00] "+f"(y00),
        [y01] "+f"(y01), [y02] "+f"(y02), [y10] "+f"(y10), [y11] "+f"(y11),
        [y12] "+f"(y12)
      : [zero] "f"(zero)
      : "memory");
}
...
void SobelRow_MMI(const uint8_t* src_sobelx,
  ...
  __asm__ volatile(
      ...
      : [src_sobelx] "+r"(src_sobelx), [src_sobely] "+r"(src_sobely),
        [dst_argb] "+r"(dst_argb), [t0] "+f"(temp[0]), [t1] "+f"(temp[1]),
        [t2] "+f"(temp[2]), [width] "+r"(width)
      : [c1] "f"(c1)
      : "memory");
}
...
void SobelToPlaneRow_MMI(const uint8_t* src_sobelx,
  ...
  __asm__ volatile(
      ...
      "nop                                             \n\t"
      : [src_sobelx] "+r"(src_sobelx), [src_sobely] "+r"(src_sobely),
        [dst_y] "+r"(dst_y), [tr] "+f"(tr), [tb] "+f"(tb), [width] "+r"(width)
      :
      : "memory");
}
...
void SobelXYRow_MMI(const uint8_t* src_sobelx,
  ...
  __asm__ volatile(
      ...
      "nop                                                \n\t"
      : [src_sobelx] "+r"(src_sobelx), [src_sobely] "+r"(src_sobely),
        [dst_argb] "+r"(dst_argb), [tr] "+f"(temp[0]), [tb] "+f"(temp[1]),
        [tg] "+f"(temp[2]), [gb] "+f"(gb), [cr] "+f"(cr), [result] "+f"(result),
        [width] "+r"(width)
      : [c1] "f"(c1)
      : "memory");
}
...
void ARGBMirrorRow_MMI(const uint8_t* src, uint8_t* dst, int width) {
  ...
  __asm__ volatile(
      ...
      "bnez    %[width], 1b                    \n\t"
      : [temp] "+f"(temp), [src] "+r"(src), [dst] "+r"(dst), [width] "+r"(width)
      : [shuff] "f"(shuff)
      : "memory");
}
...
void SplitUVRow_MMI(const uint8_t* src_uv,
  ...
  __asm__ volatile(
      ...
      : [src_uv] "+r"(src_uv), [dst_u] "+r"(dst_u), [dst_v] "+r"(dst_v),
        [width] "+r"(width), [t0] "+f"(temp[0]), [t1] "+f"(temp[1]),
        [t2] "+f"(temp[2]), [t3] "+f"(temp[3])
...
void YUY2ToUVRow_MMI(const uint8_t* src_yuy2,
  ...
  __asm__ volatile(
      ...
      "bgtz     %[width],      1b                                    \n\t"
      "nop                                                           \n\t"
      : [src_yuy2] "+r"(src_yuy2), [src_stride] "+r"(src_stride),
        [src_stride_yuy2] "+r"(src_stride_yuy2), [dst_u] "+r"(dst_u),
        [dst_v] "+r"(dst_v), [width] "+r"(width), [t0] "+f"(temp[0]),
        [t1] "+f"(temp[1]), [t2] "+f"(temp[2]), [d0] "+f"(data[0]),
...
void UYVYToUVRow_MMI(const uint8_t* src_uyvy,
  ...
  __asm__ volatile(
      ...
      "bgtz     %[width],      1b                                     \n\t"
      "nop                                                            \n\t"
      : [src_uyvy] "+r"(src_uyvy), [src_stride] "+r"(src_stride),
        [src_stride_uyvy] "+r"(src_stride_uyvy), [dst_u] "+r"(dst_u),
        [dst_v] "+r"(dst_v), [width] "+r"(width), [t0] "+f"(temp[0]),
        [t1] "+f"(temp[1]), [t2] "+f"(temp[2]), [d0] "+f"(data[0]),
        [d1] "+f"(data[1]), [d2] "+f"(data[2]), [d3] "+f"(data[3])
      : [c0] "f"(c0), [shift] "f"(shift)
...
void InterpolateRow_MMI(uint8_t* dst_ptr,
  ...
  if (source_y_fraction == 128) {
    ...
    __asm__ volatile(
        ...
        "bgtz   %[width],     1b                           \n\t"
        "nop                                               \n\t"
        : [dst_ptr] "+r"(dst_ptr), [src_ptr] "+r"(src_ptr), [uv] "+f"(uv),
          [uv_stride] "+f"(uv_stride), [width] "+r"(width)
  ...
  __asm__ volatile(
      ...
      : [src_ptr] "+r"(src_ptr), [src_ptr1] "+r"(src_ptr1),
        [dst_ptr] "+r"(dst_ptr), [fy1] "+f"(source_y_fraction), [fy0] "+f"(fy0),
        [t0] "+f"(temp), [d0] "+f"(data[0]), [d1] "+f"(data[1]),
        [d2] "+f"(data[2]), [d3] "+f"(data[3]), [c0] "+f"(c0),
        [width] "+r"(width)
...
void I422ToYUY2Row_MMI(const uint8_t* src_y,
  ...
  __asm__ volatile(
      ...
      : [src_y] "+r"(src_y), [src_u] "+r"(src_u), [src_v] "+r"(src_v),
        [dst_frame] "+r"(dst_frame), [width] "+r"(width), [ty] "+f"(temp[1]),
        [tu] "+f"(temp[1]), [tv] "+f"(temp[1]), [vu] "+f"(vu)::"memory");
}
