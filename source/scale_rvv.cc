...
// ScaleUVRowUp2_OOO_RVV function is equal to other platforms'
// ScaleRowUp2_OOO_Any_XXX. We process entire row in this function. Other
...
void ScaleRowUp2_Linear_RVV(const uint8_t* src_ptr,
                            ...
                            int dst_width) {
  // NOTE: To match behavior on other platforms, vxrm (fixed-point rounding mode
  // register) is set to round-to-nearest-up mode(0) for vnclip.
  asm volatile("csrwi vxrm, 0");
  ...
  while (src_width > 0) {
    ...
    vuint8m4_t v_src1 = __riscv_vle8_v_u8m4(work_src_ptr + 1, vl);

    vuint16m8_t v_src0_u16 = __riscv_vwaddu_vx_u16m8(v_src0, 0, vl);
    vuint16m8_t v_src1_u16 = __riscv_vwaddu_vx_u16m8(v_src1, 0, vl);
    vuint16m8_t v_tmp0 = __riscv_vwmaccu_vv_u16m8(v_src0_u16, v_3, v_src1, vl);
    vuint16m8_t v_tmp1 = __riscv_vwmaccu_vv_u16m8(v_src1_u16, v_3, v_src0, vl);

    vuint8m4_t v_dst_odd = __riscv_vnclipu_wx_u8m4(v_tmp0, 2, vl);
    vuint8m4_t v_dst_even = __riscv_vnclipu_wx_u8m4(v_tmp1, 2, vl);
...
void ScaleRowUp2_Bilinear_RVV(const uint8_t* src_ptr,
  ...
  uint8_t* work_d = d + 1;
  uint8_t* work_e = e + 1;
  size_t vl = __riscv_vsetvl_e16m4(src_width);
  ...
  while (src_width > 0) {
    ...
    v_s0_u16 = __riscv_vwaddu_vx_u16m4(v_s0, 0, vl);
    v_s1_u16 = __riscv_vwaddu_vx_u16m4(v_s1, 0, vl);
    ...
    v_dst0_odd = __riscv_vnclipu_wx_u8m2(v_t0_u16, 4, vl);
    v_dst0_even = __riscv_vnclipu_wx_u8m2(v_t1_u16, 4, vl);
    v_dst1_odd = __riscv_vnclipu_wx_u8m2(v_s0_u16, 4, vl);
    v_dst1_even = __riscv_vnclipu_wx_u8m2(v_s1_u16, 4, vl);
...
// ScaleUVRowUp2_OOO_RVV function is equal to other platforms'
// ScaleUVRowUp2_OOO_Any_XXX. We process entire row in this function. Other
// platforms only implement non-edge part of image and process edge with scalar.
void ScaleUVRowUp2_Linear_RVV(const uint8_t* src_ptr,
  ...
  while (work_width > 0) {
    size_t vl = __riscv_vsetvl_e8m4(work_width);
    vuint8m4_t v_uv0 = __riscv_vle8_v_u8m4(work_src_ptr, vl);
    vuint8m4_t v_uv1 = __riscv_vle8_v_u8m4(work_src_ptr + 2, vl);

    vuint16m8_t v_uv0_u16 = __riscv_vwaddu_vx_u16m8(v_uv0, 0, vl);
    vuint16m8_t v_uv1_u16 = __riscv_vwaddu_vx_u16m8(v_uv1, 0, vl);

    vuint16m8_t v_tmp0 = __riscv_vwmaccu_vv_u16m8(v_uv0_u16, v_3_u8, v_uv1, vl);
    vuint16m8_t v_tmp1 = __riscv_vwmaccu_vv_u16m8(v_uv1_u16, v_3_u8, v_uv0, vl);

    vuint8m4_t v_dst_odd_u8 = __riscv_vnclipu_wx_u8m4(v_tmp0, 2, vl);
    vuint8m4_t v_dst_even_u8 = __riscv_vnclipu_wx_u8m4(v_tmp1, 2, vl);

    vuint16m4_t v_dst_even = __riscv_vreinterpret_v_u8m4_u16m4(v_dst_even_u8);
    vuint16m4_t v_dst_odd = __riscv_vreinterpret_v_u8m4_u16m4(v_dst_odd_u8);
...
void ScaleUVRowUp2_Bilinear_RVV(const uint8_t* src_ptr,
  ...
  size_t vl = __riscv_vsetvl_e16m4(work_width);
  vuint16m4_t v_3_u16 = __riscv_vmv_v_x_u16m4(3, vl);
  vuint8m2_t v_3_u8 = __riscv_vmv_v_x_u8m2(3, vl);
  d[0] = (3 * s[0] + t[0] + 2) >> 2;
  e[0] = (s[0] + 3 * t[0] + 2) >> 2;
  ...
  while (work_width > 0) {
    ...
    v_dst0_odd_u8 = __riscv_vnclipu_wx_u8m2(v_t0_u16, 4, vl);
    v_dst0_even_u8 = __riscv_vnclipu_wx_u8m2(v_t1_u16, 4, vl);
    v_dst1_odd_u8 = __riscv_vnclipu_wx_u8m2(v_s0_u16, 4, vl);
    v_dst1_even_u8 = __riscv_vnclipu_wx_u8m2(v_s1_u16, 4, vl);
