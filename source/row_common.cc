...
#else
#define STATIC_CAST(type, expr) (expr)
#endif
...
void RGB565ToARGBRow_C(const uint8_t* src_rgb565,
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = STATIC_CAST(uint8_t, src_rgb565[0] >> 5) |
                STATIC_CAST(uint8_t, (src_rgb565[1] & 0x07) << 3);
    uint8_t r = src_rgb565[1] >> 3;
    dst_argb[0] = STATIC_CAST(uint8_t, b << 3) | STATIC_CAST(uint8_t, b >> 2);
    dst_argb[1] = STATIC_CAST(uint8_t, g << 2) | STATIC_CAST(uint8_t, g >> 4);
    dst_argb[2] = STATIC_CAST(uint8_t, r << 3) | STATIC_CAST(uint8_t, r >> 2);
    dst_argb[3] = 255u;
    dst_argb += 4;
    src_rgb565 += 2;
  }
...
void ARGB1555ToARGBRow_C(const uint8_t* src_argb1555,
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = STATIC_CAST(uint8_t, src_argb1555[0] >> 5) |
                STATIC_CAST(uint8_t, (src_argb1555[1] & 0x03) << 3);
    uint8_t r = (src_argb1555[1] & 0x7c) >> 2;
    uint8_t a = src_argb1555[1] >> 7;
    dst_argb[0] = STATIC_CAST(uint8_t, b << 3) | STATIC_CAST(uint8_t, b >> 2);
    dst_argb[1] = STATIC_CAST(uint8_t, g << 3) | STATIC_CAST(uint8_t, g >> 2);
    dst_argb[2] = STATIC_CAST(uint8_t, r << 3) | STATIC_CAST(uint8_t, r >> 2);
    dst_argb[3] = -a;
    dst_argb += 4;
    src_argb1555 += 2;
...
void ARGB4444ToARGBRow_C(const uint8_t* src_argb4444,
  ...
  for (x = 0; x < width; ++x) {
    ...
    dst_argb[0] = STATIC_CAST(uint8_t, b << 4) | STATIC_CAST(uint8_t, b);
    dst_argb[1] = STATIC_CAST(uint8_t, g << 4) | STATIC_CAST(uint8_t, g);
    dst_argb[2] = STATIC_CAST(uint8_t, r << 4) | STATIC_CAST(uint8_t, r);
    dst_argb[3] = STATIC_CAST(uint8_t, a << 4) | STATIC_CAST(uint8_t, a);
    dst_argb += 4;
    src_argb4444 += 2;
  }
}
...
void ARGBToRGB565Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  if (width & 1) {
    ...
    uint8_t g0 = src_argb[1] >> 2;
    uint8_t r0 = src_argb[2] >> 3;
    *(uint16_t*)(dst_rgb) = STATIC_CAST(uint16_t, b0) |
...
void ARGBToRGB565DitherRow_C(const uint8_t* src_argb,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, b0) |
                                STATIC_CAST(uint16_t, g0 << 5) |
                                STATIC_CAST(uint16_t, r0 << 11);
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, b1) |
...
void ARGBToARGB1555Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) =
        STATIC_CAST(uint16_t, b0) | STATIC_CAST(uint16_t, g0 << 5) |
        STATIC_CAST(uint16_t, r0 << 10) | STATIC_CAST(uint16_t, a0 << 15);
    *(uint16_t*)(dst_rgb + 2) =
        STATIC_CAST(uint16_t, b1) | STATIC_CAST(uint16_t, g1 << 5) |
        STATIC_CAST(uint16_t, r1 << 10) | STATIC_CAST(uint16_t, a1 << 15);
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
...
void ARGBToARGB4444Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) =
        STATIC_CAST(uint16_t, b0) | STATIC_CAST(uint16_t, g0 << 4) |
        STATIC_CAST(uint16_t, r0 << 8) | STATIC_CAST(uint16_t, a0 << 12);
    *(uint16_t*)(dst_rgb + 2) =
        STATIC_CAST(uint16_t, b1) | STATIC_CAST(uint16_t, g1 << 4) |
        STATIC_CAST(uint16_t, r1 << 8) | STATIC_CAST(uint16_t, a1 << 12);
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
...
void ABGRToAR30Row_C(const uint8_t* src_abgr, uint8_t* dst_ar30, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    *(uint32_t*)(dst_ar30) =
        STATIC_CAST(uint32_t, r0) | STATIC_CAST(uint32_t, g0 << 10) |
        STATIC_CAST(uint32_t, b0 << 20) | STATIC_CAST(uint32_t, a0 << 30);
    dst_ar30 += 4;
    src_abgr += 4;
  }
}
...
void RGB565ToYRow_C(const uint8_t* src_rgb565, uint8_t* dst_y, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = STATIC_CAST(uint8_t, src_rgb565[0] >> 5) |
                STATIC_CAST(uint8_t, (src_rgb565[1] & 0x07) << 3);
    uint8_t r = src_rgb565[1] >> 3;
    b = STATIC_CAST(uint8_t, b << 3) | STATIC_CAST(uint8_t, b >> 2);
    g = STATIC_CAST(uint8_t, g << 2) | STATIC_CAST(uint8_t, g >> 4);
    r = STATIC_CAST(uint8_t, r << 3) | STATIC_CAST(uint8_t, r >> 2);
...
void ARGB1555ToYRow_C(const uint8_t* src_argb1555, uint8_t* dst_y, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t r = (src_argb1555[1] & 0x7c) >> 2;
    b = STATIC_CAST(uint8_t, b << 3) | STATIC_CAST(uint8_t, b >> 2);
    g = STATIC_CAST(uint8_t, g << 3) | STATIC_CAST(uint8_t, g >> 2);
    r = STATIC_CAST(uint8_t, r << 3) | STATIC_CAST(uint8_t, r >> 2);
    dst_y[0] = RGBToY(r, g, b);
    src_argb1555 += 2;
    dst_y += 1;
...
void ARGB4444ToYRow_C(const uint8_t* src_argb4444, uint8_t* dst_y, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = src_argb4444[0] >> 4;
    uint8_t r = src_argb4444[1] & 0x0f;
    b = STATIC_CAST(uint8_t, b << 4) | STATIC_CAST(uint8_t, b);
    g = STATIC_CAST(uint8_t, g << 4) | STATIC_CAST(uint8_t, g);
    r = STATIC_CAST(uint8_t, r << 4) | STATIC_CAST(uint8_t, r);
    dst_y[0] = RGBToY(r, g, b);
    src_argb4444 += 2;
    dst_y += 1;
  }
...
void RGB565ToUVRow_C(const uint8_t* src_rgb565,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    uint8_t g0 = STATIC_CAST(uint8_t, src_rgb565[0] >> 5) |
                 STATIC_CAST(uint8_t, (src_rgb565[1] & 0x07) << 3);
    uint8_t r0 = src_rgb565[1] >> 3;
    uint8_t b1 = src_rgb565[2] & 0x1f;
    uint8_t g1 = STATIC_CAST(uint8_t, src_rgb565[2] >> 5) |
                 STATIC_CAST(uint8_t, (src_rgb565[3] & 0x07) << 3);
    uint8_t r1 = src_rgb565[3] >> 3;
    uint8_t b2 = next_rgb565[0] & 0x1f;
    uint8_t g2 = STATIC_CAST(uint8_t, next_rgb565[0] >> 5) |
                 STATIC_CAST(uint8_t, (next_rgb565[1] & 0x07) << 3);
    uint8_t r2 = next_rgb565[1] >> 3;
    uint8_t b3 = next_rgb565[2] & 0x1f;
    uint8_t g3 = STATIC_CAST(uint8_t, next_rgb565[2] >> 5) |
                 STATIC_CAST(uint8_t, (next_rgb565[3] & 0x07) << 3);
    uint8_t r3 = next_rgb565[3] >> 3;

    b0 = STATIC_CAST(uint8_t, b0 << 3) | STATIC_CAST(uint8_t, b0 >> 2);
    g0 = STATIC_CAST(uint8_t, g0 << 2) | STATIC_CAST(uint8_t, g0 >> 4);
    r0 = STATIC_CAST(uint8_t, r0 << 3) | STATIC_CAST(uint8_t, r0 >> 2);
    b1 = STATIC_CAST(uint8_t, b1 << 3) | STATIC_CAST(uint8_t, b1 >> 2);
    g1 = STATIC_CAST(uint8_t, g1 << 2) | STATIC_CAST(uint8_t, g1 >> 4);
    r1 = STATIC_CAST(uint8_t, r1 << 3) | STATIC_CAST(uint8_t, r1 >> 2);
    b2 = STATIC_CAST(uint8_t, b2 << 3) | STATIC_CAST(uint8_t, b2 >> 2);
    g2 = STATIC_CAST(uint8_t, g2 << 2) | STATIC_CAST(uint8_t, g2 >> 4);
    r2 = STATIC_CAST(uint8_t, r2 << 3) | STATIC_CAST(uint8_t, r2 >> 2);
    b3 = STATIC_CAST(uint8_t, b3 << 3) | STATIC_CAST(uint8_t, b3 >> 2);
    g3 = STATIC_CAST(uint8_t, g3 << 2) | STATIC_CAST(uint8_t, g3 >> 4);
    r3 = STATIC_CAST(uint8_t, r3 << 3) | STATIC_CAST(uint8_t, r3 >> 2);
...
void ARGB1555ToUVRow_C(const uint8_t* src_argb1555,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    uint8_t g0 = STATIC_CAST(uint8_t, src_argb1555[0] >> 5) |
                 STATIC_CAST(uint8_t, (src_argb1555[1] & 0x03) << 3);
    uint8_t r0 = (src_argb1555[1] & 0x7c) >> 2;
    uint8_t b1 = src_argb1555[2] & 0x1f;
    uint8_t g1 = STATIC_CAST(uint8_t, src_argb1555[2] >> 5) |
                 STATIC_CAST(uint8_t, (src_argb1555[3] & 0x03) << 3);
    uint8_t r1 = (src_argb1555[3] & 0x7c) >> 2;
    uint8_t b2 = next_argb1555[0] & 0x1f;
    uint8_t g2 = STATIC_CAST(uint8_t, next_argb1555[0] >> 5) |
                 STATIC_CAST(uint8_t, (next_argb1555[1] & 0x03) << 3);
    uint8_t r2 = (next_argb1555[1] & 0x7c) >> 2;
    uint8_t b3 = next_argb1555[2] & 0x1f;
    uint8_t g3 = STATIC_CAST(uint8_t, next_argb1555[2] >> 5) |
                 STATIC_CAST(uint8_t, (next_argb1555[3] & 0x03) << 3);
    uint8_t r3 = (next_argb1555[3] & 0x7c) >> 2;

    b0 = STATIC_CAST(uint8_t, b0 << 3) | STATIC_CAST(uint8_t, b0 >> 2);
    g0 = STATIC_CAST(uint8_t, g0 << 3) | STATIC_CAST(uint8_t, g0 >> 2);
    r0 = STATIC_CAST(uint8_t, r0 << 3) | STATIC_CAST(uint8_t, r0 >> 2);
    b1 = STATIC_CAST(uint8_t, b1 << 3) | STATIC_CAST(uint8_t, b1 >> 2);
    g1 = STATIC_CAST(uint8_t, g1 << 3) | STATIC_CAST(uint8_t, g1 >> 2);
    r1 = STATIC_CAST(uint8_t, r1 << 3) | STATIC_CAST(uint8_t, r1 >> 2);
    b2 = STATIC_CAST(uint8_t, b2 << 3) | STATIC_CAST(uint8_t, b2 >> 2);
    g2 = STATIC_CAST(uint8_t, g2 << 3) | STATIC_CAST(uint8_t, g2 >> 2);
    r2 = STATIC_CAST(uint8_t, r2 << 3) | STATIC_CAST(uint8_t, r2 >> 2);
    b3 = STATIC_CAST(uint8_t, b3 << 3) | STATIC_CAST(uint8_t, b3 >> 2);
    g3 = STATIC_CAST(uint8_t, g3 << 3) | STATIC_CAST(uint8_t, g3 >> 2);
    r3 = STATIC_CAST(uint8_t, r3 << 3) | STATIC_CAST(uint8_t, r3 >> 2);
...
void ARGB4444ToUVRow_C(const uint8_t* src_argb4444,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    b0 = STATIC_CAST(uint8_t, b0 << 4) | STATIC_CAST(uint8_t, b0);
    g0 = STATIC_CAST(uint8_t, g0 << 4) | STATIC_CAST(uint8_t, g0);
    r0 = STATIC_CAST(uint8_t, r0 << 4) | STATIC_CAST(uint8_t, r0);
    b1 = STATIC_CAST(uint8_t, b1 << 4) | STATIC_CAST(uint8_t, b1);
    g1 = STATIC_CAST(uint8_t, g1 << 4) | STATIC_CAST(uint8_t, g1);
    r1 = STATIC_CAST(uint8_t, r1 << 4) | STATIC_CAST(uint8_t, r1);
    b2 = STATIC_CAST(uint8_t, b2 << 4) | STATIC_CAST(uint8_t, b2);
    g2 = STATIC_CAST(uint8_t, g2 << 4) | STATIC_CAST(uint8_t, g2);
    r2 = STATIC_CAST(uint8_t, r2 << 4) | STATIC_CAST(uint8_t, r2);
    b3 = STATIC_CAST(uint8_t, b3 << 4) | STATIC_CAST(uint8_t, b3);
    g3 = STATIC_CAST(uint8_t, g3 << 4) | STATIC_CAST(uint8_t, g3);
    r3 = STATIC_CAST(uint8_t, r3 << 4) | STATIC_CAST(uint8_t, r3);
...
void I422ToARGB4444Row_C(const uint8_t* src_y,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_argb4444 + 0) =
        STATIC_CAST(uint16_t, b0) | STATIC_CAST(uint16_t, g0 << 4) |
        STATIC_CAST(uint16_t, r0 << 8) | STATIC_CAST(uint16_t, 0xf000);
    *(uint16_t*)(dst_argb4444 + 2) =
        STATIC_CAST(uint16_t, b1) | STATIC_CAST(uint16_t, g1 << 4) |
        STATIC_CAST(uint16_t, r1 << 8) | STATIC_CAST(uint16_t, 0xf000);
    src_y += 2;
    src_u += 1;
    src_v += 1;
...
void I422ToARGB1555Row_C(const uint8_t* src_y,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_argb1555 + 0) =
        STATIC_CAST(uint16_t, b0) | STATIC_CAST(uint16_t, g0 << 5) |
        STATIC_CAST(uint16_t, r0 << 10) | STATIC_CAST(uint16_t, 0x8000);
    *(uint16_t*)(dst_argb1555 + 2) =
        STATIC_CAST(uint16_t, b1) | STATIC_CAST(uint16_t, g1 << 5) |
        STATIC_CAST(uint16_t, r1 << 10) | STATIC_CAST(uint16_t, 0x8000);
    src_y += 2;
    src_u += 1;
    src_v += 1;
...
void I422ToRGB565Row_C(const uint8_t* src_y,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb565 + 0) =
        STATIC_CAST(uint16_t, b0) | STATIC_CAST(uint16_t, g0 << 5) |
        STATIC_CAST(uint16_t, r0 << 11);  // for ubsan
