...
void ARGB1555ToARGBRow_C(const uint8_t* src_argb1555,
  ...
  for (x = 0; x < width; ++x) {
    ...
    uint8_t g = STATIC_CAST(uint8_t, (src_argb1555[0] >> 5) | ((src_argb1555[1] & 0x03) << 3));
...
void ARGBToRGB565DitherRow_C(const uint8_t* src_argb,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, b0 | (g0 << 5) | (r0 << 11));
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, b1 | (g1 << 5) | (r1 << 11));
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
...
void ARGBToARGB1555Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, b0 | (g0 << 5) | (r0 << 10) | (a0 << 15));
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, b1 | (g1 << 5) | (r1 << 10) | (a1 << 15));
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
    ...
    uint8_t r0 = src_argb[2] >> 3;
    uint8_t a0 = src_argb[3] >> 7;
    *(uint16_t*)(dst_rgb) = STATIC_CAST(uint16_t, b0 | (g0 << 5) | (r0 << 10) | (a0 << 15));
  }
}
...
void ARGBToARGB4444Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, b0 | (g0 << 4) | (r0 << 8) | (a0 << 12));
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, b1 | (g1 << 4) | (r1 << 8) | (a1 << 12));
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
    ...
    uint8_t r0 = src_argb[2] >> 4;
    uint8_t a0 = src_argb[3] >> 4;
    *(uint16_t*)(dst_rgb) = STATIC_CAST(uint16_t, b0 | (g0 << 4) | (r0 << 8) | (a0 << 12));
  }
}
...
void ABGRToAR30Row_C(const uint8_t* src_abgr, uint8_t* dst_ar30, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    *(uint32_t*)(dst_ar30) = STATIC_CAST(uint32_t, (r0) | (g0 << 10) | (b0 << 20) | (a0 << 30));
    dst_ar30 += 4;
    src_abgr += 4;
  }
}
...
void ARGBToAR30Row_C(const uint8_t* src_argb, uint8_t* dst_ar30, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    *(uint32_t*)(dst_ar30) = STATIC_CAST(uint32_t, b0 | (g0 << 10) | (r0 << 20) | (a0 << 30));
    dst_ar30 += 4;
    src_argb += 4;
  }
}
...
void ARGBQuantizeRow_C(uint8_t* dst_argb,
  ...
  for (x = 0; x < width; ++x) {
    ...
    int g = dst_argb[1];
    int r = dst_argb[2];
    dst_argb[0] = STATIC_CAST(uint8_t, (b * scale >> 16) * interval_size + interval_offset);
    dst_argb[1] = STATIC_CAST(uint8_t, (g * scale >> 16) * interval_size + interval_offset);
    dst_argb[2] = STATIC_CAST(uint8_t, (r * scale >> 16) * interval_size + interval_offset);
    dst_argb += 4;
  }
}
...
void I422ToRGB565Row_C(const uint8_t* src_y,
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    g1 = g1 >> 2;
    r1 = r1 >> 3;
    *(uint16_t*)(dst_rgb565 + 0) = STATIC_CAST(uint16_t, (b0) | (g0 << 5) | (r0 << 11));
    *(uint16_t*)(dst_rgb565 + 2) = STATIC_CAST(uint16_t, (b1) | (g1 << 5) | (r1 << 11));
    src_y += 2;
    src_u += 1;
    src_v += 1;
...
void CumulativeSumToAverageRow_C(const int32_t* tl,
  ...
  for (i = 0; i < count; ++i) {
    dst[0] = (uint8_t)(STATIC_CAST(float, bl[w + 0] + tl[0] - bl[0] - tl[w + 0]) * ooa);
    dst[1] = (uint8_t)(STATIC_CAST(float, bl[w + 1] + tl[1] - bl[1] - tl[w + 1]) * ooa);
    dst[2] = (uint8_t)(STATIC_CAST(float, bl[w + 2] + tl[2] - bl[2] - tl[w + 2]) * ooa);
    dst[3] = (uint8_t)(STATIC_CAST(float, bl[w + 3] + tl[3] - bl[3] - tl[w + 3]) * ooa);
    dst += 4;
    tl += 4;
    bl += 4;
  }
...
static void HalfRow_16To8_C(const uint16_t* src_uv,
  ...
  int x;
  for (x = 0; x < width; ++x) {
    dst_uv[x] = STATIC_CAST(uint8_t, C16TO8((src_uv[x] + src_uv[src_uv_stride + x] + 1) >> 1, scale));
  }
}
