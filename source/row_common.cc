...
#if altmethod
  uint32_t y1 = (uint32_t)(y * 0x0101) >> 2;  // convert to 2.14 fixed point
  yg *= 4;  // make into 2.10 fixed point scale
  *b = Clamp((int32_t)(y1 - (u * ub)          + bb) * yg >> 16);
  *g = Clamp((int32_t)(y1 - (u * ug + v * vg) + bg) * yg >> 16);
  *r = Clamp((int32_t)(y1           -(v * vr) + br) * yg >> 16);
#endif
