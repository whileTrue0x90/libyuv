...
static void HalfRow_16to8_C(const uint16_t* src_uv,
  ...
  for (x = 0; x < width; ++x) {
    dst_uv[x] =
        clamp255(((src_uv[x] + src_uv[src_uv_stride + x] + 1) >> 1) * scale) >>
        16;
  }
}
...
void InterpolateRow_16to8_C(uint8_t* dst_ptr,
  ...
  for (x = 0; x < width - 1; x += 2) {
    dst_ptr[0] =
        clamp255(((src_ptr[0] * y0_fraction + src_ptr1[0] * y1_fraction) >> 8) *
                 scale) >>
        16;
    dst_ptr[1] =
        clamp255(((src_ptr[1] * y0_fraction + src_ptr1[1] * y1_fraction) >> 8) *
                 scale) >>
        16;
    src_ptr += 2;
    src_ptr1 += 2;
