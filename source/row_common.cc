...
void ARGBToARGB1555Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, (b0) | (g0 << 5)) |
                                STATIC_CAST(uint16_t, (r0 << 10) | (a0 << 15));
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, (b1) | (g1 << 5)) |
                                STATIC_CAST(uint16_t, (r1 << 10) | (a1 << 15));
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
    ...
    uint8_t r0 = src_argb[2] >> 3;
    uint8_t a0 = src_argb[3] >> 7;
    *(uint16_t*)(dst_rgb) = STATIC_CAST(uint16_t, (b0) | (g0 << 5)) |
                            STATIC_CAST(uint16_t, (r0 << 10) | (a0 << 15));
  }
}
...
void ARGBToARGB4444Row_C(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint16_t*)(dst_rgb + 0) = STATIC_CAST(uint16_t, (b0) | (g0 << 4)) |
                                STATIC_CAST(uint16_t, (r0 << 8) | (a0 << 12));
    *(uint16_t*)(dst_rgb + 2) = STATIC_CAST(uint16_t, (b1) | (g1 << 4)) |
                                STATIC_CAST(uint16_t, (r1 << 8) | (a1 << 12));
    dst_rgb += 4;
    src_argb += 8;
  }
  if (width & 1) {
    ...
    uint8_t r0 = src_argb[2] >> 4;
    uint8_t a0 = src_argb[3] >> 4;
    *(uint16_t*)(dst_rgb) = STATIC_CAST(uint16_t, (b0) | (g0 << 4)) |
                            STATIC_CAST(uint16_t, (r0 << 8) | (a0 << 12));
  }
}
...
void ABGRToAR30Row_C(const uint8_t* src_abgr, uint8_t* dst_ar30, int width) {
  ...
  for (x = 0; x < width; ++x) {
    ...
    *(uint32_t*)(dst_ar30) = STATIC_CAST(uint32_t, (r0) | (g0 << 10)) |
                             STATIC_CAST(uint32_t, (b0 << 20) | (a0 << 30));
    dst_ar30 += 4;
    src_abgr += 4;
  }
}
