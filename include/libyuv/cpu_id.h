...
// the auto init. |cpu_flags| should be a valid combination of the kCpuHas
// constants above and include kCpuInitialized. Use this method when running in
// a sandboxed process where InitCpuFlags might fail (as it might access
// /proc/cpuinfo). In such cases the cpu_info can be obtained from a non
// sandboxed process by calling InitCpuFlags() and passed to the sandboxed
// process (via command line parameters, IPC...) which can then call this method
